...
busted.analysis_description = {terms.io.info : "BUSTED-SRV (branch-site unrestricted statistical test of episodic diversification with synonymous rate variation) uses a random effects branch-site model fitted jointly to all or a subset of tree branches in order to test for alignment-wide evidence of episodic diversifying selection. Assuming there is evidence of positive selection (i.e. there is an omega > 1), BUSTED will also perform a quick evidence-ratio style analysis to explore which individual sites may have been subject to selection.",
...
busted.SRV_rate = "SRV_rate";
busted.SRV_weight = "SRV_weight";
...
function busted.model.srv.ModelDescription (type, code, components) {
    return utility.Extend (models.codon.BS_REL.ModelDescription (type, code, components), 
                          {terms.model.rate_variation : 
                               rate_variation.types.GDD.factory (
                               {
                                    terms.rate_variation.bins : busted.srv_rate_classes
                               })
                          }
                          );
}
...
function busted.srv.ExtractMixtureDistribution (srv){
    ...
    for (i = 1; i <= count; i+=1) {
        rates [i-1] = ((srv[utility.getGlobalValue ("terms.parameters")])[utility.getGlobalValue ("terms.global")])["GDD rate category " + i];
        if (i < count ) {
            weights [i-1] = ((srv[utility.getGlobalValue ("terms.parameters")])[utility.getGlobalValue ("terms.global")])["Mixture auxiliary weight "+ i ];
        }
    }
...
busted.test.bsrel_model =  model.generic.DefineMixtureModel("busted.model.srv.ModelDescription",
        "busted.test", {
...
busted.background.bsrel_model =  model.generic.DefineMixtureModel("busted.model.srv.ModelDescription",
        "busted.background", {
            "0": parameters.Quote(terms.global),
            "1": busted.codon_data_info[terms.code],
            "2": parameters.Quote (busted.rate_classes), // the number of rate classes
